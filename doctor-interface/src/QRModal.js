  ...
  render () {
    return (
      <Modal isOpen={this.props.visibility} toggle={this.toggle}>
        <ModalHeader>Scanează adresa pacientului</ModalHeader>
        <ModalBody>
          <div>
            { this.state && this.state.result ?
              <p><FontAwesome name='check-circle'/> {this.state.result}<br/>
                {this.state.count.toString()} prescripțiile precedente ale pacientului</p> :
            ...
            { this.state.showScanner ?
              ...
              <Button color="secondary" className="col text-center align-middle ml-1" onClick={() => { this.rescan() }}><FontAwesome name='repeat'/> Scanează din nou</Button>
              </div>
            }
          </div>
        </ModalBody>
